...
url1 = "https://www.achrafothman.net/aslsmt/corpus/sample-corpus-asl-en.asl"
url2 = "https://www.achrafothman.net/aslsmt/corpus/sample-corpus-asl-en.en"
...
with open("Seq2Seq.csv", "w", newline="", encoding="utf-8") as csvfile:
    ...
    writer.writerow(["gloss", "text"]) # header row
...
with open("Seq2Seq80.csv", "w" , encoding="utf-8", newline='') as f:
    ...
    #crea el archivo Seq2Seq20-SLG, que contiene el 20% final del dataset, unicamente la parte en ESL
with open("Seq2Seq20-SLG.csv", "w", encoding="utf-8", newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Seq2Seq20slg']) # write the new header row with the first column name
    writer.writerows(set_20_slg)

    #crea el archivo Seq2Seq20-ENG, que contiene el 20% final del dataset, unicamente la parte en ingles
with open("Seq2Seq20-ENG.csv", "w", encoding="utf-8", newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Seq2Seq20eng']) # write the new header row with the first column name
    writer.writerows(set_20_eng)
